...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    ...
    await request(server).get('/api/security-dashboard/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/security-dashboard/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/security-dashboard/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/security-dashboard/todos/${createdTodoItem.id}`)
